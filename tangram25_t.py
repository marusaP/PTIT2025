...
#
# - there is 7 tiles, numbered 1 - 7
# - use LEFT mouse click to move tiles around, use RIGHT mouse click to rotate the tile
...
# - with "Send figure" button poses of tile centers are sent to the robot (TO DO !!!)
...
def triangle_inradius(vertices):
    ax, ay = vertices[0]
    bx, by = vertices[1]
    cx, cy = vertices[2]
...
# Triangle properties
green_triangle_points = [(0, 0), (18 * TILE_SIZE*K, 0), (9 * TILE_SIZE*K, 9 * TILE_SIZE*K)]
green_triangle_center = triangle_incenter(green_triangle_points)
green_triangle_radius = triangle_inradius(green_triangle_points)

blue_triangle_points = [(0, 0), (9 * TILE_SIZE*K, 0), (9 * TILE_SIZE*K, 9 * TILE_SIZE*K)]
blue_triangle_center = triangle_incenter(blue_triangle_points)
blue_triangle_radius = triangle_inradius(blue_triangle_points)

small_triangle_points = [(0, 0), (9 * TILE_SIZE*K, 0), (4.5 * TILE_SIZE*K, 4.5 * TILE_SIZE*K)]
small_triangle_center = triangle_incenter(small_triangle_points)
small_triangle_radius = triangle_inradius(small_triangle_points)

# Square and parallelogram properties
square_size = (9 / 2) * TILE_SIZE*K * math.sqrt(2)
...
parallelogram_points = [(0, 0), (9 * TILE_SIZE*K, 0), (13.5 * TILE_SIZE*K, 4.5 * TILE_SIZE*K), (4.5 * TILE_SIZE*K, 4.5 * TILE_SIZE*K)]
...
parallelogram_radius = min(9 * TILE_SIZE*K / 2, 4.5 * TILE_SIZE *K/ 2)
...
# def rotate_point(point, angle, center): # levosucni!
...
def rotate_point(point, angle, center):
    angle_rad = math.radians(-angle)  # Negative angle for counterclockwise rotation
    x, y = point
    cx, cy = center
    new_x = cx + (x - cx) * math.cos(angle_rad) - (y - cy) * math.sin(angle_rad)
    new_y = cy + (x - cx) * math.sin(angle_rad) + (y - cy) * math.cos(angle_rad)
...
def display_right_area_positions():
    ...
    for shape, rotation in zip(shapes, rotations):
        ...
        if center_x > LEFT_AREA_WIDTH + THICK_LINE_WIDTH:
            ...
            positions_raw.extend((shape['number'], round(relative_x/100, 2), round(relative_y/100, 2), corrected_rotation)) # to send
        else:
            positions_raw.extend((shape['number'],0,0,0))
            #positions_raw.extend((0,0,0,0))
    
    # sort
    pos_sort=np.zeros(28)
    for i in range(0,7):
        k = positions_raw[4*i]
        # print(k)
        # print(pos_sort)
        # print(positions_raw)
        if k != 0:
            pos_sort[4*(k-1):4*(k-1)+4] = positions_raw[4*i:4*i+4]
    # print(pos_sort)
    # print(positions_raw)
    return positions, pos_sort

# check if tiles (circles) overlap after translation: 
def check_overlap(shapes, moved_shape_idx, newcenter_x, newcenter_y):
    moved_shape_center = (newcenter_x, newcenter_y)
    moved_shape_radius = shapes[moved_shape_idx]['radius']
    for i, shape in enumerate(shapes):
        if i != moved_shape_idx:
            distance = math.hypot(moved_shape_center[0] - shape['position'][0], moved_shape_center[1] - shape['position'][1])
            if distance < moved_shape_radius + shape['radius']:
                return True
    return False
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:  # Left mouse button
                for i, shape in enumerate(shapes): 
                    if math.hypot(event.pos[0] - shape['position'][0], event.pos[1] - shape['position'][1]) < shape['radius']: # Check if the mouse is in the inscribed circle
                        dragging[i] = True
                        mouse_x, mouse_y = event.pos
                        offset_x = shape['position'][0] - mouse_x
                        offset_y = shape['position'][1] - mouse_y
                if reset_button_rect.collidepoint(event.pos): # Reset tiles button
                    reset_tile_positions()
                elif send_figure_button_rect.collidepoint(event.pos): # Send figure button
                    positions, pos_raw = display_right_area_positions()
                    ...
                    # 
                    # 
                    #      DOPOLNI!  izvedi pošiljanje pos_raw robotu
                    # 
                    # 
                    # 
                    # 
                ...
                elif display_tile_number_rect.collidepoint(event.pos): # Checkbox for displaying tile number
                    display_tile_number = not display_tile_number
                elif display_pose_rect.collidepoint(event.pos): # Checkbox for displaying tile pose
                    display_pose = not display_pose
                elif show_circles_rect.collidepoint(event.pos): # Checkbox for displaying inscribed circles
                ...
                for i, shape in enumerate(shapes):
                    if math.hypot(event.pos[0] - shape['position'][0], event.pos[1] - shape['position'][1]) < shape['radius']: # Check if the mouse is in the inscribed circle
                        ...
                        if rotations[i] >= 360:
                            rotations[i] -= 360
        elif event.type == pygame.MOUSEBUTTONUP: # Button released
            if event.button == 1:  # Left mouse button
                dragging = [False] * len(shapes)
            ...
            for i, shape in enumerate(shapes):
                if dragging[i]:
                    mouse_x, mouse_y = event.pos
                    if not check_overlap(shapes, i, mouse_x+offset_x, mouse_y+offset_y): # Check if the tiles overlap after translation
                        shapes[i]['position'] = (mouse_x + offset_x, mouse_y + offset_y)
